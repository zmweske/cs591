...
class connection_thread():
    def __init__(self, port, uuid, user, message_sender, incoming_messages):
        ...
        # self.messages_broadcaster.put(json.dumps({"user": self.user, "uuid": self.uuid, "action": "QUIT", "data": "Logged out"}))
        LOGGING_OUT.put(self.uuid)
        self.client_socket.close()
        logging.info("Closed server connection " + str(self.port))# + "...")
    ...
    def send_to_client(self):
        while not self.stop:
            ...
            try:
                ...
                logging.warning("91: " + type(e).__name__ + ' ' + str(e))
    ...
    def listen_to_broadcast(self):
        while not self.stop:
            ...
            if json_msg["uuid"] == self.uuid:
                continue
            message = json_msg["user"] + {False:": ",True:" "}[json_msg["action"]=="CONNECT"] + json_msg["data"]
                ...
                try:
                    ...
                    try:  # TODO convert to check action?
                        if message == "QUIT" or message == '\x04':
                            ...
                            message = json.dumps({"user": self.user, "uuid": self.uuid, "action": "QUIT", "data": "Logged off"})
                            message = message.encode()
